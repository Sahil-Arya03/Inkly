/* Signup screen */
const { useState: useStateSignup, useMemo: useMemoSignup } = React;
...
function Signup({ onSignedUp, onNavLogin, theme, onTheme }) {
  const [name, setName]     = useStateSignup("Avery Chen");
  const [email, setEmail]   = useStateSignup("");
  const [pw, setPw]         = useStateSignup("");
  const [showPw, setShowPw] = useStateSignup(false);
  const [workspace, setWorkspace] = useStateSignup("northwind");
  const [plan, setPlan]     = useStateSignup("team");
  const [agree, setAgree]   = useStateSignup(false);
  const [updates, setUpdates] = useStateSignup(true);
  const [errs, setErrs]     = useStateSignup({});
  const [loading, setLoading] = useStateSignup(false);

  const strength = useMemoSignup(() => scorePassword(pw), [pw]);
  ...
  function submit(e) {
    ...
    setLoading(true);
    setTimeout(() => { setLoading(false); onSignedUp(); }, 1100);
  }
      ...
      <div className="login__right">
        ...
        <form className="auth" onSubmit={submit} noValidate>
          ...
          <div className="auth__oauth">
            <button type="button" className="oauth-btn"><Icons.G/> Sign up with Google</button>
            <button type="button" className="oauth-btn"><Icons.Microsoft/> Sign up with Microsoft</button>
          </div>
          ...
          <div className="auth__row" style={{flexDirection: "column", alignItems: "flex-start", gap: 8, marginTop: 16}}>
            <label className="checkbox">
              ...
              I agree to the <a href="#" onClick={e => e.preventDefault()} style={{marginLeft: 4}}>Terms</a><span style={{margin: "0 4px", color: "var(--ink-4)"}}>·</span><a href="#" onClick={e => e.preventDefault()}>Privacy</a>
            </label>
            <label className="checkbox">
          ...
          <div className="auth__footer">
            ...
            <a href="#" onClick={e => { e.preventDefault(); onNavLogin && onNavLogin(); }}>Sign in</a>
          </div>
        </form>
      </div>
    </div>
