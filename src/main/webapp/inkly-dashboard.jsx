...
function DashStat({ label, num, delta, dir, sub }) {
      ...
      {events.map((e, i) => (
        <div key={i} className={`dash-sched__row dash-sched__row--${e.state}`}>
          ...
          <div className="dash-sched__pill-slot">
            {e.state === "next" && <span className="dash-sched__pill">Next · {e.when}</span>}
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="page dash">
      ...
      <section className="dash__hero">
        ...
        <div className="dash__hero-actions">
          <button className="btn btn--ghost"><I.Sparkle s={13} /> Daily plan</button>
      ...
      <section className="dash__body">
        <div className="dash__col-l">
          <DashSection title="Today's schedule" action="OPEN CALENDAR">
            <DashSchedule />
          </DashSection>
