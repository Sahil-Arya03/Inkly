...
const INBOX_KIND_ICON = {
  mention:  (p) => <window.I s={p?.s||14} {...p}><path d="M16 8a4 4 0 1 0-4 4 4 4 0 0 0 4-4Zm0 0v2a3 3 0 0 0 6 0v-1a10 10 0 1 0-4 8" /></window.I>,
  assign:   (p) => <window.I s={p?.s||14} {...p}><path d="M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z" /><path d="M2 21a7 7 0 0 1 14 0" /><path d="M19 8v6m3-3h-6" /></window.I>,
  review:   (p) => <window.I s={p?.s||14} {...p}><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z" /><circle cx="12" cy="12" r="3" /></window.I>,
  comment:  (p) => <window.I s={p?.s||14} {...p}><path d="M4 6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H9l-4 4v-4H6a2 2 0 0 1-2-2V6Z" /></window.I>,
  deploy:   (p) => <window.I s={p?.s||14} {...p}><path d="M5 18a8 8 0 0 1 14-6 5 5 0 0 1-3 9H8a5 5 0 0 1-3-3Z" /><path d="m9 12 3-3 3 3M12 9v8" /></window.I>,
  deadline: (p) => <window.I s={p?.s||14} {...p}><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></window.I>,
  status:   (p) => <window.I s={p?.s||14} {...p}><path d="M4 12h8M4 6h16M4 18h12" /><path d="m17 9 4 3-4 3" /></window.I>,
  doc:      (p) => <window.I s={p?.s||14} {...p}><path d="M14 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8l-5-5Z" /><path d="M14 3v5h5" /></window.I>,
};
...
function InboxDetail({ item, onPushToast }) {
  ...
  const sender = peopleById(item.who);
  const IconK = INBOX_KIND_ICON[item.kind];
            ...
            <div key={i} className="cmt">
              ...
              <div className="cmt__body">
                ...
                <div className="cmt__text">
                  {m.mention
                    ? <span dangerouslySetInnerHTML={{__html: m.text.replace(/@(\w+)/g, '<span class="inb-mention">@$1</span>')}}/>
                    : m.text
                  }
                </div>
              </div>
...
function InboxPage({ onPushToast }) {
  const I = window.Icons;
  const [items, setItems] = useStateInbox(INBOX_ITEMS);
  const [filter, setFilter] = useStateInbox("all"); // all|unread|mentions|assigned|review
  const [selectedId, setSelectedId] = useStateInbox(INBOX_ITEMS[0].id);
  const [query, setQuery] = useStateInbox("");
